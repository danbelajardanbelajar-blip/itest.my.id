<div class="dashboard-view fade-in">
    <div class="view-header flex-between">
        <div>
            <h1>Edit Kelas</h1>
            <p>Perbarui informasi data kelas.</p>
        </div>
    ...
    <div class="admin-recent-section glass-panel" style="max-width: 600px;">
        <form id="editClassForm" onsubmit="submitForm(event, this, '<?= url('admin/updateClass/' . $class->id) ?>')">
            <div style="margin-bottom: 20px;">
                ...
                <input type="text" name="level" class="form-input" style="width: 100%; padding: 12px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: #fff;" required value="<?= htmlspecialchars($class->level) ?>">
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="text" name="name" class="form-input" style="width: 100%; padding: 12px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: #fff;" required value="<?= htmlspecialchars($class->name) ?>">
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <select name="teacher_id" class="form-input" style="width: 100%; padding: 12px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: #fff;">
                    ...
                    <?php if(!empty($teachers)): foreach($teachers as $teacher): ?>
                        <option value="<?= $teacher->id ?>" style="background: #1e1e2d;" <?= ($class->teacher_id == $teacher->id) ? 'selected' : '' ?>><?= htmlspecialchars($teacher->name) ?> (<?= htmlspecialchars($teacher->nip ?? '-') ?>)</option>
