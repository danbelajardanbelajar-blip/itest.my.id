<div class="dashboard-view fade-in">
    <div class="view-header">
        ...
        <p>Perbarui data mata pelajaran yang sudah ada.</p>
    </div>

    <div class="admin-recent-section glass-panel" style="max-width: 600px;">
        <form action="<?= url('admin/updateSubject/' . $subject->id) ?>" method="POST" class="ajax-form">
            ...
            <div style="display: grid; grid-template-columns: 1fr; gap: 20px; margin-bottom: 24px;">
                <div>
                    ...
                    <input type="text" name="code" value="<?= htmlspecialchars($subject->code ?? '') ?>" required placeholder="Contoh: MTK" style="width: 100%; padding: 10px 14px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: #fff;">
                </div>
                <div>
                    <label style="display: block; margin-bottom: 5px; color: var(--text-color); font-weight: 500;">Nama Mata Pelajaran</label>
                    <input type="text" name="name" value="<?= htmlspecialchars($subject->name) ?>" required placeholder="Contoh: Matematika" style="width: 100%; padding: 10px 14px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: #fff;">
                </div>
            </div>
